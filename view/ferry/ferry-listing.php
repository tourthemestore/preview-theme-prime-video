...
<div class="c-containerDark">
  ...
  <div class="container">
    ...
    <div class="row">
        ...
        <div class="accordion c-accordion filterPriceSidebar" id="filterPrice">
          ...
          <div class="filterItem">
            ...
            <div class="dropdown selectable">
              <select id="price_filter_id" name="price_filter_id" title="Select Destination" class="form-control" style="width:100%" onchange="get_price_filter_data('ferry_result_array',this.id,0,0);">
          ...
          <div class="filterItem">
            ...
            <div class="checkboxLists" id="cruise_types">
            </div>
          </div>
          <hr />
          <div class="filterItem">
            ...
            <div class="checkboxLists" id="cruise_classes">
            </div>
          </div>
          <hr />
      ...
      while (($row_query  = mysqli_fetch_assoc($query))) {
        ...
        if ($sq_tariff_count > 0) {
          ...
          while (($row_tariff  = mysqli_fetch_assoc($sq_tariff))) {
            ...
            $sq_from = mysqli_fetch_assoc(mysqlQuery("select currency_rate from roe_master where currency_id='$row_tariff[currency_id]'"));
            ...
            $total_cost1 = (float)(($adult * $adult_cost) + ($children * $child_cost) + ($infant * $infant_cost));

            $adult_cost1 = 1 / $from_currency_rate *  (float) $adult_cost;
            $child_cost1 = 1 / $from_currency_rate * (float)($child_cost);
            $infant_cost1 = 1 / $from_currency_rate * (float)($infant_cost);
            $c_amount1 = 1 / $from_currency_rate * $total_cost1;
            ...
            array_push($all_costs_array, array('amount' => $c_amount1, 'id' => $row_tariff['currency_id']));
      ...
      //   $total_cost = ($ferry_result_array[$i]['total_cost']);
      ...
      //   $from_currency_rate = $sq_from['currency_rate'];
      //   $c_amount = 1 / $from_currency_rate  * $total_cost;
      // }
      ...
      for ($i = 0; $i < sizeof($ferry_result_array); $i++) {
        ...
        if (!in_array($ferry_result_array[$i]['ferry_category'], $ferry_class_array)) {

          array_push($ferry_class_array, $ferry_result_array[$i]['ferry_category']);
        }
        if (!in_array($ferry_result_array[$i]['ferry_type'], $ferry_type_array)) {

          array_push($ferry_type_array, $ferry_result_array[$i]['ferry_type']);
        }
      }
      sort($ferry_class_array);
      sort($ferry_type_array);
      ?>
      ...
      <input type="hidden" value='<?= json_encode($ferry_class_array, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE) ?>' id="ferry_class_array" />
      <input type="hidden" id="selected_ferry_class_array" />
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('body').delegate('.lblfilterChk', 'click', function() {
      get_price_filter_data('ferry_result_array', 'price_filter_id', 0, 0);
    });
    $('body').delegate('.lblftfilterChk', 'click', function() {
      get_price_filter_data('ferry_result_array', 'price_filter_id', 0, 0);
    });
  ...
  function get_price_filter_data(ferry_result_array, type1, fromRange_cost, toRange_cost, flag = true) {
    ...
    setTimeout(() => {
      ...
      var ferry_class_array = [];
      var final_arr1 = [];

      var checkboxes = document.getElementsByName('ferry_class');
      for (var checkbox of checkboxes) {
        if (checkbox.checked)
          ferry_class_array.push(checkbox.value);
      }

      if (ferry_class_array.length != 0) {
        final_arr1 = (final_arr).filter(function(a) {
          return ferry_class_array.includes(a.ferry_category);
        });
      } else {
        final_arr1 = final_arr;
      }
      $('#selected_ferry_class_array').val(ferry_class_array);
      ...
      if (ferry_type_array.length != 0) {
        final_arr2 = (final_arr1).filter(function(a) {
          return ferry_type_array.includes(a.ferry_type);
        });
      } else {
  ...
  function get_price_filter_data_result(final_arr) {
    ...
    $.post(base_url + 'view/ferry/ferry_results.php', {
      final_arr: final_arr
    }, function(data) {
  ...
  function getSliderValue() {
    ...
    if (ranges[0] != '' && ranges[1] != '' && ranges[0] !== 'NaN' && ranges[1] !== 'NaN') {
      get_price_filter_data('ferry_result_array', 'price_filter_id', ranges[0], ranges[1], true);
    }
  }
