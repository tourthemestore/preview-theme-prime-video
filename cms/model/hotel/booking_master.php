...
class booking_master
{
  ...
  public function booking_save()
  {
    ...
    foreach ($bsmValues[0] as $key => $value) {
      switch ($key) {
        case 'basic':
          $sub_total = ($value != "") ? $value : $sub_total;
          break;
        case 'service':
    ...
    if ((float)($discount) > ((float)($service_charge) + (float)($markup))) {
  ...
  public function booking_update()
  {
    ...
    if ($sq_booking) {
      ...
      /////////////////// // Update entries log//////////////////////
      global $transaction_master;
      if ((float)($old_total) != (float)($total_fee)) {
  ...
  public function hotel_booking_email_send($booking_id, $total_fee, $payment_amount, $credit_card_amount)
  {
    ...
    $total_fee1 = currency_conversion($currency, $sq_hotel_booking['currency_code'], ($total_fee + (float)($credit_card_amount)));
    $payment_amount1 = currency_conversion($currency, $sq_hotel_booking['currency_code'], ($payment_amount + (float)($credit_card_amount)));
