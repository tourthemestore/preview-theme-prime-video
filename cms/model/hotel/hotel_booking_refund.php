...
class hotel_booking_refund{
	...
	public function finance_save($refund_id){
		...
		$guest_name = (($sq_cust['type']=='Corporate' || $sq_cust['type'] == 'B2B') && $sq_hotel_info['pass_name']!='') ? '('.$sq_hotel_info['pass_name'].')' : '';
		...
		$particular = 'Payment through '.$refund_mode.' for '.get_hotel_booking_id($booking_id,$yr1).' for '.$sq_hte['no_of_nights'].' Night(s) stay from '.get_date_user($sq_hte['check_in']).' in '.$sq_hte['category'].' Room at '.$hotel_name .' * '.(intval($sq_hotel_info['adults'])+intval($sq_hotel_info['childrens'])).' '.$cust_name.$guest_name;
	...
	public function refund_mail_send($booking_id,$refund_amount,$refund_date,$refund_mode,$transaction_id){
		...
		$ac_refund_amount = $sq_sq_train_info['refund_total_fee'];
		$sq_ref_pay_total = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from hotel_booking_refund_master where booking_id='$booking_id' and clearance_status!='Cancelled'"));
		$remaining_pay = $ac_refund_amount - $sq_ref_pay_total['sum'];
		$remaining_show = ($remaining_pay < 0) ? 0 : $remaining_pay;
		$refund_amount1 = currency_conversion($currency,$sq_sq_train_info['currency_code'],$remaining_show);
		...
		<tr>
			<table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
				...
				<tr><td style="text-align:left;border: 1px solid #888888;">Pending Refund Amount</td>   <td style="text-align:left;border: 1px solid #888888;">'.$refund_amount1.'</td></tr>
			</table>
		</tr>';
		$content .= '</tr>';
