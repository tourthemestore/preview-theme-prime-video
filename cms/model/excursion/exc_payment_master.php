...
class exc_payment_master
{
	...
	public function whatsapp_send()
	{
		...
		$total_pay_amt = (float)($sq_pay['sum']) + (float)($sq_pay['sumc']);
		$total_amount = (float)($sq_ticket_info['exc_total_cost']) + (float)($sq_pay['sumc']);
		...
		if ($sq_entries == $sq_entries_cancel) {
			...
			$outstanding = ($total_pay_amt > $canc_amount) ? 0 : (float)($canc_amount) - (float)($total_pay_amt) + (float)($sq_pay['sumc']);
		} else {
			$outstanding =  (float)($total_amount) - (float)($total_pay_amt);
		}
