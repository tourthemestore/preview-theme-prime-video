...
<table class="table table-hover" id="tbl_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_refund = mysqli_fetch_assoc($sq_estimate)){
			...
			$estimate_type_val = get_estimate_type_name($query['estimate_type'], $query['estimate_type_id']);
			$vendor_type_val = get_vendor_name($query['vendor_type'], $query['vendor_type_id']);
