...
$remaining_pay = $refund_amount - $sq_refund_amount1['sum'];
?>
            ...
            <div class="widget_parent">
                <div class="stat_content main_block">
                    ...
                    <span class="main_block content_span" data-original-title="" title="">
                        ...
                        <span class="stat_content-amount pull-right" data-original-title=""
                            title=""><?php echo number_format($remaining_pay, 2); ?></span>
                    </span>
                </div>
            </div>
        </div>
            ...
            <div class="panel panel-default panel-body mg_bt_10">
                ...
                <div class="row  text-center">
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        ...
                        <input type="hidden" name="remaining_pay" id="remaining_pay"
                            value="<?php echo $remaining_pay; ?>">
                    </div>
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                ...
                <table class="table table-hover table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
                    ...
                    <tbody>
						...
						while ($row_exc_refund = mysqli_fetch_assoc($sq_exc_refund)) {
							...
							$sq_exc_info = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master where exc_id='$row_exc_refund[exc_id]' and delete_status='0'"));

							$cust_name = "";
							$sq_refund_entries = mysqlQuery("select * from exc_refund_entries where refund_id='$row_exc_refund[refund_id]'");
							if ($row_exc_refund['clearance_status'] == 'Pending') {
								$bg = "warning";
							} else if ($row_exc_refund['clearance_status'] == 'Cancelled') {
								$bg = "danger";
							} else if ($row_exc_refund['clearance_status'] == 'Cleared') {
								$bg = "success";
							} else {
								$bg = "";
							}
							...
							while ($row_refund_entry = mysqli_fetch_assoc($sq_refund_entries)) {
								$sq_cust = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master where customer_id='$row_refund_entry[entry_id]' and delete_status='0'"));
								...
								$url = BASE_URL . "model/app_settings/generic_refund_voucher_pdf.php?v_voucher_no=$v_voucher_no&booking_id=" . get_exc_booking_id($exc_id, $year) . "&v_refund_date=$v_refund_date&v_refund_to=$v_refund_to&v_service_name=$v_service_name&v_refund_amount=$v_refund_amount&v_payment_mode=$v_payment_mode&customer_id=$customer_id&refund_id=$refund_id&currency_code=$sq_exc_info[currency_code]";
						?>
                        <tr class="<?= $bg ?>">
                            <td><?= $count ?></td>
                            <td><?= $cust_name ?></td>
                    ...
                    <tfoot>
                        ...
                        <tr class="active">
                            ...
                            <th colspan="2" class="success">Total_Refund :
                                <?= number_format($refund_amount1 - $pend_amount - $can_amount, 2); ?></th>
                        </tr>
                    </tfoot>
                </table>
            </div>
                ...
                callback: function(data1) {
                    if (data1 == "yes") {
                        ...
                        $.ajax({
                            type: 'post',
                            url: base_url +
                                'controller/excursion/refund/refund_save.php',
                            data: {
                                exc_id: exc_id,
