...
while($row_exc_payment = mysqli_fetch_assoc($sq_exc_payment)){
	...
	if($row_exc_payment ['currency_code'] ==''){
		$currency_code_2 = $sq_exc_info['currency_code'];
	  }
	  else{
		$currency_code_2= $row_exc_payment ['currency_code'] ;
	  }

	  $pay_id= $row_exc_payment['payment_id'];

  $net_amt= $total_sale;

	$url1 = BASE_URL."model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=$branch_status&outstanding=$outstanding&table_name=exc_payment_master&customer_field=exc_id&in_customer_id=$row_exc_payment[exc_id]]&currency_code=$currency_code_2&status=$row_exc_payment[status]&pay_id=$pay_id&net_amt=$net_amt";
	$checshow = "";
