			...
			<table class="table table-hover no-marg-sm" id="tbl_att_list">
				...
				<tbody>
					...
					while ($row_emp = mysqli_fetch_assoc($sq_emp)) {
						...
						++$count;
						$bg = "";
						$sq_att_count = mysqli_num_rows(mysqlQuery("select * from employee_attendance_log where att_date='$attendence_date' and emp_id='$row_emp[emp_id]'"));
						if($sq_att_count > 0){
							...
							if ($sq_att['status'] == "Present") {
								$bg = "success";
							} else if ($sq_att['status'] == "Absent") {
								$bg = "danger";
							} else if ($sq_att['status'] == "On Tour" || $sq_att['status'] == "Half Day" || $sq_att['status'] == "Work From Home" || $sq_att['status'] == "Holiday Off" || $sq_att['status'] == "Weekly Off") {
								$bg = "warning";
							}
						}
					?>
...
<div class="row mg_tp_20 text-center">
	<div class="col-md-4 col-md-offset-4 col-xs-4">
		<button id="btn_att_save" class="btn btn-sm btn-success" onclick="user_attendence_Save()"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Save</button>
	</div>
</div>
...
<script>
	...
	function user_attendence_Save() {
		...
		if (status_value == '0') {
			error_msg_alert("Atleast select one user attendance");
			return false;
		}
