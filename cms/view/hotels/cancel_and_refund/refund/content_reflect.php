            ...
            <div class="panel panel-default panel-body mg_bt_10">
                <div class="row text-center">
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        <input type="hidden" name="remaining_fund" id="remaining_fund"
            ...
            <table class="table table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
                ...
                <tbody>
					...
					while ($row_hotel_refund = mysqli_fetch_assoc($sq_hotel_refund)) {
						...
						$sq_hotel_info1 = mysqli_fetch_assoc(mysqlQuery("select * from hotel_booking_master where booking_id='$row_hotel_refund[booking_id]' and delete_status='0'"));
						...
						if ($row_hotel_refund['clearance_status'] == 'Pending') {
							...
							$bg = "danger";
						} else if ($row_hotel_refund['clearance_status'] == 'Cleared') {
							$bg = "success";
						} else {
							$bg = "";
						}
                ...
                <tfoot>
					...
					?>
                    <tr class="active">
                        <th class="text-right info" colspan="2">Refund :
                            <?= ($sq_pay['sum'] == "") ? 0 : $sq_pay['sum'] ?></th>
                        <th class="text-right warning" colspan="2">Pending clearence:
                            <?= ($sq_ref_pen_total['sum'] == "") ? 0 : $sq_ref_pen_total['sum'] ?></th>
                        <th class="text-right danger" colspan="2">Cancelled :
                            <?= ($sq_ref_can_total['sum'] == "") ? 0 : $sq_ref_can_total['sum'] ?></th>
                        <th class="text-right success" colspan="2">Total_Refund :
                            <?= $sq_pay['sum'] - $sq_ref_pen_total['sum'] - $sq_ref_can_total['sum'] ?></th>
                    </tr>
                </tfoot>
            </table>
        </div>
                ...
                callback: function(data1) {
                    if (data1 == "yes") {
                        ...
                        $.ajax({
                            type: 'post',
                            url: base_url +
                                'controller/hotel/refund/refund_save.php',
                            data: {
