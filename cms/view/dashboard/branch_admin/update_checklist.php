...
<div class="modal fade profile_box_modal" id="bview_modal" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
    ...
    <div class="modal-content" style="text-align:center!important;">
        ...
        <input type="hidden" id="tour_type" name="tour_type" value="<?= $tour_type ?>">
                ...
                { 
                    ...
                    $sql=mysqlQuery("select * from to_do_entries where entity_id='$row_entity[entity_id]'");
                        ...
                        while($sq_todo_list=mysqli_fetch_assoc($sql))
                        {
                            ...
                            <tr class="<?= $bg ?>">
                                <td>
                                    <input type="checkbox" id="chk_package_tour_checklist_<?= $sq_todo_list['id'] ?>" name="chk_package_tour_checklist" <?= $chk_status ?> data-entity-id="<?= $sq_todo_list['id'] ?>" checked>
                                </td>
                                <td><?= $count ?></td>
                                <td><?= $sq_todo_list['entity_name'] ?></td>
                                <td class="<?= $class1 ?>"><select name="assigned_emp_id<?= $sq_todo_list['id'] ?>" id="assigned_emp_id<?= $sq_todo_list['id'] ?>" title="Assigned To" style="width:100%">
                                    <?php
            ...
            <div class="row text-center mg_tp_20">
                <div class="col-md-12">
                    <button class="btn btn-sm btn-success" id="bupdate_checklist"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Update</button>
                </div>
            </div>
        </form>
        <?php }else{
...
$('#frm_emquiry_save').validate({
    ...
    $('#bupdate_checklist').prop('disabled',true);
    ...
    if(entity_id_arr.length == 0){
        ...
        $('#bupdate_checklist').prop('disabled',false);
        $('#bupdate_checklist').button('reset');
        return false;
    }
    ...
    $('#bupdate_checklist').button('loading');
