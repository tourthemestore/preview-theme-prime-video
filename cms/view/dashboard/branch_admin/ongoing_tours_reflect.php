    ...
    <div class="col-md-12">
        <div class="col-md-12 no-pad table_verflow"> 
            ...
            if($sq_cancel_count != $sq_count){
                ...
                <!-- Hotel Booking -->
                <?php                   
                $query1 = "select *	from hotel_booking_entries where status!='Cancel' and DATE(check_in)<= '$today' and DATE(check_out) >= '$today' and booking_id in (select booking_id from hotel_booking_master where delete_status='0')";
                $sq_query = mysqlQuery($query1);
                while($row_query=mysqli_fetch_assoc($sq_query)){
                ...
                <!-- Bus Booking -->
                <?php
                $sq_query_bus = mysqlQuery("select * from bus_booking_entries where DATE(date_of_journey)	= '$today' and status!='Cancel' and booking_id in(select booking_id from bus_booking_master where delete_status='0')");
                ...
                <!-- Excursion Booking -->
                <?php
                $sq_query_exc = mysqlQuery("select * from excursion_master_entries where DATE(exc_date) ='$today' and status!='Cancel'");
                while($row_query1=mysqli_fetch_assoc($sq_query_exc)){

                    $sq_c = mysqli_num_rows(mysqlQuery("select * from excursion_master where exc_id = '$row_query1[exc_id]' and delete_status='0'"));
                    if($sq_c != 0){
