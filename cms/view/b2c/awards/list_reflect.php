...
$query = mysqlQuery("SELECT * FROM `b2c_awards` order by entry_id desc");
?>
	...
	while($row_query = mysqli_fetch_assoc($query)){
		...
		?>
			<tr class="<?= $bg ?>">
				...
				<td><?= $title ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="update_modal(<?= $row_query['entry_id'] ?>)" title="Update Details" id="update_btn-<?= $row_query['entry_id'] ?>"><i class="fa fa-pencil-square-o"></i></button>
				</td>
			</tr>
		<?php
		} ?>
