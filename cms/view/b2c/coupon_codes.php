                ...
                <table id="tbl_coupons" name="tbl_coupons" class="table border_0 table-hover" style="width:1000px;">
                    ...
                    <tr>
                        ...
                        <td><select name="amount_in-1" id="amount_in-1<?= $i ?>_u" title="Amount In"
                                ...
                                <option value="<?= $coupon_codes[$i]->amount_in ?>"><?= $coupon_codes[$i]->amount_in ?>
                                </option>
                                <option value="">*Amount In</option>
                                <option value="Percentage">Percentage</option>
                                <option value="Flat">Flat</option>
                    ...
                    <script>
                    $('#valid_date-1<?= $i ?>_u').datetimepicker({
                        timepicker: false,
                        minDate: new Date(),
                        format: 'd-m-Y'
                    });
...
function hotel_names_load(id, offset) {
    ...
    var city_id = $('#' + id).val();
    $.get(base_url + 'view/hotels/booking/inc/hotel_name_load.php', {
        city_id: city_id
    }, function(data) {
        ...
        submitHandler: function(form) {
            ...
            for (var i = 0; i < rowCount; i++) {
                ...
                if (row.cells[0].childNodes[0].checked) {
                    ...
                    if (coupon_code == "") {
                        error_msg_alert("Enter coupon code at row " + (i + 1));
                        return false;
                    }
                    if (amount_in == "") {
                        error_msg_alert("Select amount-in at row " + (i + 1));
                        return false;
                    }
                    if (amount == "") {
                        error_msg_alert("Enter amount at row " + (i + 1));
                        return false;
                    }
                    if (valid_date == "") {
