...
public function quotation_master_save()
{
	...
	$sq_quotation = mysqlQuery("INSERT INTO flight_quotation_master ( quotation_id, enquiry_id, login_id, branch_admin_id,financial_year_id, emp_id,customer_name,  email_id, mobile_no,country_code,whatsapp_no,subtotal,markup_cost,markup_cost_subtotal,service_tax,service_charge,quotation_cost,quotation_date,bsm_values,roundoff,created_at,status,currency_code) VALUES ('$quotation_id','$enquiry_id','$login_id', '$branch_admin_id','$financial_year_id', '$emp_id', '$customer_name','$email_id','$whatsapp_no','$country_code','$mobile_no','$subtotal','$markup_cost','$markup_cost_subtotal','$service_tax','$service_charge','$total_tour_cost','$quotation_date','$bsmValues','$roundoff','$created_at','1','$currency_code')");
...
*Departure D/T* : ". get_datetime_user($row['dapart_time'])."
*Arrival D/T* : ". get_datetime_user($row['arraval_time'])."
...
*Quotation Cost* : '.$sq_quotation['quotation_cost'].$currency_amount.'
