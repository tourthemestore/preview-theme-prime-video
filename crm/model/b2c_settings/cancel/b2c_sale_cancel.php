...
class b2c_sale_cancel{
    ...
    function cancel_mail_send($booking_id)
    {
        ...
        if($sq_tour['service']=='Activity'){
            $tour_name = $enq_data[0]->act_name;
             $tour_date = $enq_data[0]->act_sate;

        }else if($sq_tour['service']=='Transfer'){
            $tour_name = $enq_data[0]->trans_name;
             $tour_date = $enq_data[0]->pickup_sate;

        }else if($sq_tour['service']=='Hotel'){
            $tour_name = $enq_data[0]->hotel_name;
             $checkin_date = $enq_data[0]->checkin;
               $checkout_date = $enq_data[0]->checkout;
                 $tour_date = $checkin_date .' To '.$checkout_date ;
