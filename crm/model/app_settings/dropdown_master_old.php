...
function get_customer_hint($branch_status = 'no', $fetchEmail = 'no', $fetchMobile = 'no')
{
  $final_array = array();
  ...
  } else if($branch_status == 'enq') {
      
    $sq_query = mysqlQuery("select * from enquiry_master where 1");
    while ($row_cust = mysqli_fetch_assoc($sq_query)) {
        
        $to_be_push = array(
            "customer_id" => $row_cust['customer_id'],
            "value" => $row_cust['email_id'],
            "label" => $row_cust['email_id'],
