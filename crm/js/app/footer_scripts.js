...
function cust_branch_reflect() {
  ...
  $.post(
    base_url + 'view/load_data/cust_branch_reflect.php',
    { cust_id: cust_id },
    function (data) {
      $('#branch_id_filter').html(data);
    }
...
//**Dual button
$(function () {
  $('.app_dual_button input[type="checkbox"], .app_dual_button input[type="radio"]').change(
    function () {
...
function customer_save_modal(client_modal_type = 'other') {
  ...
  $.post(
    base_url + 'view/customer_master/save_modal.php',
    { client_modal_type: client_modal_type },
    function (data) {
...
function generic_city_save_modal(modal_type = '') {
  ...
  $.post(
    base_url + 'view/other_masters/cities/save_modal.php',
    ...
    $(element).select2({
      ...
      ajax: {
        ...
        data: function (params) {
          return { term: params.term, page: params.page || 0, valueasText: valueasText };
        },
        processResults: function (data) {
          let more = data.pagination;
          return {
...
function payment_amount_validate(
  payment_amount_id,
  payment_mode_id,
  transaction_id_id,
  bank_name_name,
...
function event_airport(id, fromSectornum = 2, toSectornum = 3) {
  //driver function
...
function airport_load_main(ids) {
  ids.forEach(function (id) {
    ...
    $('#' + object_id)
      ...
      return $('<li disabled>')
        .append(
          '<a>' + item.value.split(' -')[1] + '<br><b>' + item.value.split(' -')[0] + '<b></a>'
        )
        .appendTo(ul);
    };
  });
...
function generic_tax_reflect(src_id, desc_id, funct_call, offset = '', temp_data = '') {
  ...
  $.post(
    base_url() + 'view/load_data/generic_tax_reflect.php',
...
function display_description(type, entry_id) {
  var base_url = $('#base_url').val();
  $.post(
    base_url + 'view/load_data/module_description_modal.php',
    { entry_id: entry_id, type: type },
    function (data) {
      $('#div_content_modal').html(data);
    }
  );
...
function pagination_load(
  ...
  if (footer_string) {
    ...
    for (
      var i = 0;
      i < parseInt(dataset_main[dataset_main.length - 1].footer_data['total_footers']);
      i++
    ) {
      html +=
        '<th class=" ' +
...
function get_identifier_block(identifier, payment_mode, credit_card_details, credit_charges) {
  ...
  if (payment_mode === 'Credit Card') {
    ...
    credit_card_company &&
      credit_card_company.filter((data) => {
...
function get_credit_card_charges(
  identifier,
  payment_mode,
  payment_amount,
  credit_card_details,
...
function add_itinerary(dest_id1, spa, dwp, ovs, dayp) {
  ...
  $.post(
    base_url + 'view/load_data/itinerary_modal.php',
    { dest_id: dest_id, spa: spa, dwp: dwp, ovs: ovs, dayp: dayp },
    function (data) {
...
function customer_whatsapp_send(first_name, contact_no, email_id, company_name, cust_type) {
  var base_url = $('#base_url').val();
  $.post(
    base_url + 'controller/customer_master/whatsapp_send.php',
    {
