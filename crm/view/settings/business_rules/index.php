...
<div class="header_bottom">
	<div class="row text-center">
		...
		<?php
		$app_query = "SELECT b2c_flag FROM app_settings LIMIT 1";
		$app_result = mysqli_fetch_assoc(mysqlQuery($app_query));

		// Check generic_count_master
		$generic_query = "SELECT setup_type FROM generic_count_master LIMIT 1";
		$generic_result = mysqli_fetch_assoc(mysqlQuery($generic_query));

		// Condition check
		if ($app_result['b2c_flag'] == 1 || $generic_result['setup_type'] == 4):
		?>
...
<script>
	...
	function values_load(id) {
		...
		if (condition === "2") {
			...
			$.post('business_rules/get_condition_values.php', {
				condition: condition
			}, function(data) {
	...
	function target_amount_change(id, type) {
		...
		switch (value) {
			case 'Exclusive':
				document.getElementById("target_amount").options[col].disabled = false;
				break;
			case 'Inclusive':
				document.getElementById("target_amount").options[col].disabled = true;
				break;
		}
	}
</script>
