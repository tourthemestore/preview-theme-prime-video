...
<form id="section_enqbk">
    ...
    <div class="row mg_bt_20">
        <div class="col-md-6 mg_bt_10">
            <select name="p_gateway" id="p_gateway" title="Select Payment Gateway" class='form-control'
                style='width:100%' onchange="get_credentials()" data-toggle="tooltip" required>
                <?php if ($p_gateway != '') { ?>
        ...
        submitHandler: function(form) {
            ...
            if (button_type === "book") {
                ...
                if (p_gateway == "RazorPay") {
                    ...
                    if (api_key == '') {
                        error_msg_alert("Enter API Key");
                        return false;
                    }
                    if (api_secret == '') {
                        error_msg_alert("Enter API Secret");
                        return false;
                    }
                    cred_array.push({
                        'api_key': api_key,
                ...
                if (p_gateway == "CCAvenue") {
                    ...
                    if (merchant_id == '') {
                        error_msg_alert("Enter Merchant ID");
                        return false;
                    }
                    if (access_code == '') {
                        error_msg_alert("Enter Access Code");
                        return false;
                    }
                    if (enc_key == '') {
                        error_msg_alert("Enter Encryption Key");
                        return false;
                    }
                    cred_array.push({
