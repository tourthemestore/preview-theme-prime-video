...
<div class="row">
    <div class="col-xs-12">
        ...
        <table id="tbl_package_tour_quotation_dynamic_train" name="tbl_package_tour_quotation_dynamic_train" class="table table-bordered pd_bt_51 no-marg">
        	...
        	if($sq_train_count==0){
				...
				<tr>
		            ...
		            <td class="col-md-2 no-pad"><input type="text" id="train_departure_date1" name="train_departure_date" placeholder="Departure Date and time" title="Departure Date and time" class="app_datetimepicker" onchange="get_to_datetime(this.id,'train_arrival_date1')" value="<?= date('d-m-Y H:i') ?>" style="width:100%"></td>
		            <td class="col-md-2 no-pad"><input type="text" id="train_arrival_date1" name="train_arrival_date" placeholder="Arrival Date and time" title="Arrival Date and time" class="app_datetimepicker" value="<?= date('d-m-Y H:i') ?>" onchange="validate_validDatetime('train_departure_date1',this.id)" style="width:100%"></td>
	            </tr>   	              
		            <script>
        		...
        		while($row_q_train = mysqli_fetch_assoc($sq_q_train)){
					...
					<tr>
		                ...
		                <td class="col-md-3 no-pad"><select id="train_from_location<?= $count ?>_1" onchange="validate_location('train_to_location<?= $count ?>_1' , 'train_from_location<?= $count ?>_1')" class="app_select2 form-control train_from" name="train_from_location" title="From Location" style="width:100%">
									<option value="<?= $row_q_train['from_location'] ?>"><?= $row_q_train['from_location'] ?></option>
			                		<option value="">From</option>
				            </select>
		                </td>
		                 <td class="col-md-3 no-pad"><select id="train_to_location<?= $count ?>_1" onchange="validate_location('train_from_location<?= $count ?>_1' , 'train_to_location<?= $count ?>_1')" class="app_select2 form-control train_to" name="train_to_location" title="To Location" style="width:100%">
		                 	    <option value="<?= $row_q_train['to_location'] ?>"><?= $row_q_train['to_location'] ?></option>
			                	<option value="">To</option>	
			            </select></td>
			            <td class="col-md-2 no-pad"><select name="train_class" id="train_class1" class="form-control" title="Class" style="width:100%">
			            ...
			            <td class="col-md-2 no-pad"><input type="text" id="train_departure_date<?= $count ?>_u" name="train_departure_date" placeholder="Departure Date and time" title="Departure Date and time" class="app_datetimepicker" onchange="get_to_datetime(this.id,'train_arrival_date<?= $count ?>_u')" value="<?= get_datetime_user($row_q_train['departure_date']) ?>" style="width:100%"></td>
			            <td class="col-md-2 no-pad"><input type="text" id="train_arrival_date<?= $count ?>_u" name="train_arrival_date" placeholder="Arrival Date and time" title="Arrival Date and time" class="app_datetimepicker" value="<?= get_datetime_user($row_q_train['arrival_date']) ?>" onchange="validate_validDatetime('train_departure_date<?= $count ?>_u',this.id)" style="width:100%"></td>
