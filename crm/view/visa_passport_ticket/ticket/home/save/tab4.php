		...
		submitHandler:function(form){
	        ...
	        for(var i=0; i<rowCount; i++){
				...
				if(row.cells[0].childNodes[0].checked){
					...
					console.log("Trip Details :"+trip_details);
			...
			console.log("Financial Year ID: "+financial_year_id);
			...
			function saveData(){
				$.ajax({
					...
					data: {
						emp_id : safe(emp_id),
						customer_id : safe(customer_id),
						tour_type : safe(tour_type),
						type_of_tour : safe(type_of_tour),
						adults : safe(adults, 0),
						childrens : safe(childrens, 0),
						infant : safe(infant, 0),
						adult_fair : safe(adult_fair, 0),
						children_fair : safe(children_fair, 0),
						infant_fair : safe(infant_fair, 0),
						basic_cost : safe(basic_cost, 0),
						markup : safe(markup, 0),
						discount : safe(discount, 0),
						yq_tax : safe(yq_tax, 0),
						other_taxes : safe(other_taxes, 0),
						service_charge : safe(service_charge, 0),
						service_tax_subtotal : safe(service_tax_subtotal, 0),
						service_tax_markup : safe(service_tax_markup, 0),
						tds : safe(tds, 0),
						due_date : safe(due_date),
						ticket_total_cost : safe(ticket_total_cost, 0),
						payment_date : safe(payment_date),
						payment_amount : safe(payment_amount, 0),
						payment_mode : safe(payment_mode),
						bank_name : safe(bank_name),
						transaction_id : safe(transaction_id),
						bank_id : safe(bank_id),
						first_name_arr : safe(first_name_arr, []),
						middle_name_arr : safe(middle_name_arr, []),
						last_name_arr : safe(last_name_arr, []),
						adolescence_arr : safe(adolescence_arr, []),
						seat_no_arr : safe(seat_no_arr, []),
						meal_plan_arr : safe(meal_plan_arr, []),
						ticket_no_arr : safe(ticket_no_arr, []),
						gds_pnr_arr : safe(gds_pnr_arr, []),
						baggage_info_arr : safe(baggage_info_arr, []),
						branch_admin_id : safe(branch_admin_id),
						financial_year_id : safe(financial_year_id),
						reflections : safe(reflections, []),
						main_ticket_arr : safe(main_ticket_arr, []),
						bsmValues : safe(bsmValues, []),
						roundoff : safe(roundoff, 0),
						credit_charges : safe(credit_charges, 0),
						credit_card_details : safe(credit_card_details),
						ticket_reissue : safe(ticket_reissue, 0),
						control : 'Sale',
						quotation_id : safe(quotation_id, 0),
						canc_policy : safe(canc_policy),
						guest_name : safe(guest_name),
						trip_details_arr1 : safe(trip_details_arr1, []),
						booking_date : safe(booking_date),
						currency_code: safe(currency_code)
					},
