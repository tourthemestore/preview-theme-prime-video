...
$query = "select estimate_type, estimate_type_id, vendor_type, vendor_type_id,status, purchase_date as date, net_total as credit, '' as debit,purchase_return as purchase_return1,cancel_estimate as cancel_estimate1,estimate_id , '' as payment_mode from vendor_estimate where financial_year_id='$financial_year_id' and delete_status='0' and status!='Cancel'";
if ($estimate_type != "") {
...
while ($row_report = mysqli_fetch_assoc($sq_estimate)) {
	...
	$temp_arr = array("data" => array(
		...
		$row_report['payment_mode'],
		number_format((float)($actual_purchase), 2),
		$row_report['debit']
	), "bg" => $bg);
