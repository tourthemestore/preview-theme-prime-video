...
$opening_bal = isset($sq_bank['balance']) ? $sq_bank['balance'] : 0;

$transaction_bal = 0;
if(($from_date!="" && $to_date!="")){
	$sq_credit = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from bank_cash_book_master where payment_date<'$from_date' and payment_type='Bank' and payment_side='Credit' and  clearance_status!='Cancelled' and bank_id='$bank_id'"));
	$sq_debit = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from bank_cash_book_master where payment_date<'$from_date' and payment_type='Bank' and payment_side='Debit' and  clearance_status!='Cancelled' and bank_id='$bank_id'"));

	// $transaction_bal = $sq_credit['sum'] - $sq_debit['sum'];	
	$transaction_bal = $sq_debit['sum'] - $sq_credit['sum'];	
}
