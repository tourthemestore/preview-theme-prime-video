...
$adult_cost = (float)($result[1]);
$chwob_cost = (float)($result[2]);
$chwb_cost = (float)($result[3]);
$infant_cost = (float)($result[4]);
$extrab_cost = (float)($result[5]);
...
$final_arr_json = $_POST['final_arr'];
$final_arr = json_decode($final_arr_json, true);
$room_count = is_array($final_arr) ? count($final_arr) : 0;
$adults_arr = [];
$children_arr = [];
...
if (is_array($final_arr)) {
    foreach ($final_arr as $item) {
        if (isset($item['rooms']) && is_array($item['rooms'])) {
            $adults_arr[] = $item['rooms']['adults'] ?? 0;
            $children_arr[] = $item['rooms']['child'] ?? 0;
        }
    }
}

$tariff_entries_id =  $_POST['tariff_entries_id'];
...
$timing_slot  = $_POST['timing_slot'];
$act_total_cost = $_POST['act_total_cost'];
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="frm_pdf_save" class="needs-validation" novalidate>
                    ...
                    <input type="hidden" id="act_date" name="act_date" value="<?= $act_date ?>" />
                    <input type="hidden" id="timing_slot" name="timing_slot" value="<?= $timing_slot ?>" />
                    <input type="hidden" id="transfer_option" name="transfer_option" value="<?= $transfer_option ?>" />

                    <input type="hidden" id="act_name" name="act_name" value="<?= $act_name ?>" />
                    ...
                    <!-- transfer -->
                    <input type="hidden" id="tariff_entries_id" name="tariff_entries_id" value="<?= $tariff_entries_id  ?>" />
                    ...
                    <!-- Hotel -->
                    <input type="hidden" id="hotel_name" name="hotel_name" value="<?= $hotel_name  ?>" />
                    <input type="hidden" id="hotel_id" name="hotel_id" value="<?= $hotel_id  ?>" />
                    <input type="hidden" id="check_in" name="check_in" value="<?= $check_in ?>" />
                    <input type="hidden" id="check_out" name="check_out" value="<?= $check_out ?>" />
                    <input type="hidden" id="total_rooms" name="total_rooms" value="<?= $total_rooms ?>" />
                    <input type="hidden" id="room_cat" name="room_cat" value="<?= $room_cat ?>" />
                    ...
                    <div class="form-row">
                        ...
                        <?php if ($type == '3') { ?>
                            <div class="form-group col-md-4" style="border-right:2px solid gray;">
                                <label> Hotel Name: <?= $hotel_name ?> </label>
                            </div>
                            <div class="form-group col-md-4" style="border-right:2px solid gray;">
                                <label> CheckIn/CheckOut Date : <?= $check_in . ' To ' . $check_out ?> </label>
                            </div>
                        ...
                        <?php } else if ($type == '4') { ?>
                            <div class="form-group col-md-4" style="border-right:2px solid gray;">
                                <label> Activity Name: <?= $act_name ?> </label>
                            </div>
                            <div class="form-group col-md-4" style="border-right:2px solid gray;">
                                <label> Activity Date: <?= $act_date ?> </label>
                            </div>

                            <div class="form-group col-md-4">
                                <label> Total Guest(s): <?= intval($adults) + intval($child) + intval($infant) ?> </label>
                            </div>
                            <!-- Transfer -->
                        <?php } else if ($type == '5') { ?>
                            <div class="form-group col-md-4" style="border-right:2px solid gray;">
                                <label> Transfer Name: <?= $trans_name ?> </label>
                            </div>
                            <div class="form-group col-md-4" style="border-right:2px solid gray;">
                                <label> Pickup Date: <?= $pickup_date ?> </label>
                            </div>

                            <div class="form-group col-md-4">
                                <label> Total Guest(s): <?= $pass ?> </label>
                            </div>
                    ...
                    <?php } else if ($type == '4') { ?>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                <input type="text" class="form-control" placeholder="Pickup Location" id="pickup_from" name="pickup_from" title="Pickup Location" />
                            </div>
                            <div class="form-group col-md-4">
                                <input type="text" class="form-control" placeholder="Pickup Date&Time" id="pickup_time" name="pickup_time" title="Pickup&Time" />
                            </div>
                            <div class="form-group col-md-4">
                                <input type="text" class="form-control" placeholder="Dropoff Location" id="drop_to" name="drop_to" title="Dropoff Location" />
                            </div>
                        </div>
                        <!-- Transfer -->
                    <?php
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                <input type="text" class="form-control" placeholder="Pickup Location" id="pickup_from" name="pickup_from" title="Pickup Location" value="<?= $pickup ?>" />
                            </div>
                            <?php if (!empty($return_date) && $return_date != 'NA') { ?>
                                <div class="form-group col-md-4">
                                    <input type="text" class="form-control" placeholder="Pickup Date&Time" id="pickup_time" name="pickup_time" title="Return Date&Time" value="<?= $return_date ?>" />
                                </div>
                            <?php } ?>

                            <div class="form-group col-md-4">
                                <input type="text" class="form-control" placeholder="Dropoff Location" id="drop_to" name="drop_to" title="Dropoff Location" value="<?= $drop ?>" />
                            </div>
                        </div>
                    <?php } ?>
                    <script>
                    ...
                    <?php if ($type == '3') { ?>
                        ...
                        <?php
                        for ($i_room = 0; $i_room < sizeof($final_arr); $i_room++) {
                            $room_number = $final_arr[$i_room]['rooms']['room'];
                        ?>
                            <div class="section">
                                <h4 class="title">Room - <?= $room_number ?></h4>
                                <?php for ($j = 1; $j <= $final_arr[$i_room]['rooms']['adults']; $j++) { ?>

                                    <label>Adult-<?= $j ?></label>
                                    <div class="form-row">
                                        <div class="form-group col-md-2">
                                            <select class="full-width" id="ahonorific<?= $i_room ?>_<?= $j ?>" title="Select Honorific" data-toggle="tooltip">
                                        ...
                                        <div class="form-group col-md-4">
                                            <input type="text" class="form-control" placeholder="<?= ($type == '4') ? 'First Name' : 'First Name' ?>" id="afirst_name<?= $i_room ?>_<?= $j ?>" name="afirst_name<?= $i_room ?>_<?= $j ?>" title="First Name" />
                                        </div>
                                        <div class="form-group col-md-4">
                                            <input type="text" class="form-control" placeholder="<?= ($type == '4') ? 'Last Name' : 'Last Name' ?>" id="alast_name<?= $i_room ?>_<?= $j ?>" name="alast_name<?= $i_room ?>_<?= $j ?>" title="Last Name" />
                                        </div>
                                        <div class="form-group col-md-2">
                                            <input type="text" class="form-control" placeholder="Birthdate" id="abirthdate<?= $i_room ?>_<?= $j ?>" name="abirthdate<?= $i_room ?>_<?= $j ?>" title="Birthdate" />
                                        </div>
                                    </div>
                                    <script>
                                        $('#abirthdate<?= $i_room ?>_<?= $j ?>').datetimepicker({
                                            format: 'd-m-Y',
                                            maxDate: yest,
                                            timepicker: false
                                        });
                                ...
                                <?php } ?>
                                <!-- For Child -->
                                <?php for ($k = 1; $k <= $final_arr[$i_room]['rooms']['child']; $k++) { ?>
                                    <label>Child<?= $k ?></label>
                                    <div class="form-row">
                                        <div class="form-group col-md-2">
                                            <select class="full-width" id="ohonorific<?= $i_room ?>_<?= $k ?>" title="Select Honorific" data-toggle="tooltip">
                                                <option value="Mr.">Mr.</option>
                                                <option value="Mrs.">Mrs.</option>
                                                <option value="Ms.">Ms.</option>
                                            </select>
                                        ...
                                        <div class="form-group col-md-4">
                                            <input type="text" class="form-control" placeholder="First Name" id="ofirst_name<?= $i_room ?>_<?= $k ?>" name="ofirst_name<?= $i_room ?>_<?= $k ?>" title="First Name" />
                                        </div>
                                        <div class="form-group col-md-4">
                                            <input type="text" class="form-control" placeholder="Last Name" id="olast_name<?= $i_room ?>_<?= $k ?>" name="olast_name<?= $i_room ?>_<?= $k ?>" title="Last Name" />
                                        </div>
                                        <div class="form-group col-md-2">
                                            <input type="text" class="form-control" placeholder="Birthdate" id="obirthdate<?= $i_room ?>_<?= $k ?>" name="obirthdate<?= $i_room ?>_<?= $k ?>" title="Birthdate" />
                                        </div>
                                    </div>
                                    <script>
                                        $('#obirthdate<?= $i_room ?>_<?= $k ?>').datetimepicker({
                                            format: 'd-m-Y',
                                            maxDate: yest,
                                            timepicker: false
                                        });
                    ...
                    <?php } else { ?>
                        ...
                        ?>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-4">
                                    <input type="text" class="form-control" placeholder="<?= ($type == '4') ? 'First Name' : '*First Name' ?>" id="afirst_name<?= $i ?>" name="afirst_name<?= $i ?>" title="First Name" <?= ($type != '4') ? 'required' : '' ?> />
                                </div>
                                <div class="form-group col-md-4">
                                    <input type="text" class="form-control" placeholder="<?= ($type == '4') ? 'Last Name' : '*Last Name' ?>" id="alast_name<?= $i ?>" name="alast_name<?= $i ?>" title="Last Name" <?= ($type != '4') ? 'required' : '' ?> />
                                </div>
                                <div class="form-group col-md-2">
                        ...
                        <?php if ($type == '4') {
                            ...
                            ?>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group col-md-4">
                                        <input type="text" class="form-control" placeholder="First Name" id="wfirst_name<?= $i ?>" name="wfirst_name<?= $i ?>" title="First Name" />
                                    </div>
                                    <div class="form-group col-md-4">
                                        <input type="text" class="form-control" placeholder="Last Name" id="wlast_name<?= $i ?>" name="wlast_name<?= $i ?>" title="Last Name" />
                                    </div>
                                    <div class="form-group col-md-2">
                                        <input type="text" class="form-control" placeholder="Birthdate" id="wbirthdate<?= $i ?>" name="wbirthdate<?= $i ?>" title="Birthdate" />
                                    </div>
                        ...
                        ?>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-4">
                                    <input type="text" class="form-control" placeholder="<?= ($type == '4') ? 'First Name' : '*First Name' ?>" id="ifirst_name<?= $i ?>" name="ifirst_name<?= $i ?>" title="First Name" <?= ($type != '4') ? 'required' : '' ?> />
                                </div>
                                <div class="form-group col-md-4">
                                    <input type="text" class="form-control" placeholder="<?= ($type == '4') ? 'Last Name' : '*Last Name' ?>" id="ilast_name<?= $i ?>" name="ilast_name<?= $i ?>" title="Last Name" <?= ($type != '4') ? 'required' : '' ?> />
                                </div>
                                <div class="form-group col-md-2">
                    ...
                    <div class="form-row">
                        ...
                        <?php if ($type == '4') {   ?>
                            <div class="form-group col-md-3">
                                ...
                                <input type="text" class="form-control" id="total_cost" placeholder="*Total Cost" data-bs-toggle="tooltip" data-bs-placement="top" title="<?= $act_total_cost ?>" readonly />
                            </div>
                            <div class="form-group col-md-4">
                    ...
                    <div class="form-row">
                        ...
                        ?>
                            <div class="form-group col-md-3">
                                <label>*Total Cost</label>
                                <input type="text" class="form-control" id="total_cost" placeholder="*Total Cost" data-bs-toggle="tooltip" data-bs-placement="top" value="<?= $trans_total_cost ?>" readonly />
                            </div>

                        <?php } else if ($type == '3') { ?>
                            <div class="form-group col-md-3">
                                <label>*Total Cost</label>
                                <input type="text" class="form-control" id="total_cost" placeholder="*Total Cost" data-bs-toggle="tooltip" data-bs-placement="top" value="<?= $total_cost ?>" readonly />
                            </div>
    ...
    $(function() {
        $('#frm_pdf_save').validate({
            ...
            submitHandler: function(form) {
                ...
                //Basic Details
                var pickup_from = (type == '1' || type == '4' || type == '5') ? $('#pickup_from').val() : '';
                var pickup_time = (type == '1' || type == '4' || type == '5') ? $('#pickup_time').val() : '';
                var drop_to = (type == '1' || type == '4' || type == '5') ? $('#drop_to').val() : '';
                ...
                var total_rooms = <?= $room_count ?>;
                var adult_counts = <?= json_encode($adults_arr) ?>;
                var child_counts = <?= json_encode($children_arr) ?>;
                let rooms_data = [];
                ...
                for (let i = 0; i < total_rooms; i++) {
                    ...
                    let adult_count = adult_counts[i];
                    let child_count = child_counts[i];

                    // Adults
                    for (let j = 1; j <= adult_count; j++) {
                        let adult = {
                            honorific: $('#ahonorific' + i + '_' + j).val(),
                            first_name: $('#afirst_name' + i + '_' + j).val(),
                            last_name: $('#alast_name' + i + '_' + j).val(),
                            birthdate: $('#abirthdate' + i + '_' + j).val()
                        };
                        room_obj.adults.push(adult);
                    }

                    // Children
                    for (let k = 1; k <= child_count; k++) {
                        let child = {
                            honorific: $('#ohonorific' + i + '_' + k).val(),
                            first_name: $('#ofirst_name' + i + '_' + k).val(),
                            last_name: $('#olast_name' + i + '_' + k).val(),
                            birthdate: $('#obirthdate' + i + '_' + k).val()
                        };
                        room_obj.children.push(child);
                    }

                    rooms_data.push(room_obj);
                }

                // View or use the final JSON
                $('#guest_details_json').val(JSON.stringify(rooms_data)); // optional: store in hidden field
